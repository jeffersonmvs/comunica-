...
(() => {
  ...
  async function initLogin() {
    const sectors = await api('/api/users/sectors');
    const sel = $('#login-sector');
    sel.innerHTML = sectors.map((s) => `<option value="${s.key}">${esc(s.label)}</option>`).join('');
    ...
    $('#login-btn').addEventListener('click', async () => {
      ...
      try {
        const user = await api('/api/users', { method: 'POST', body: JSON.stringify({ name, sector }) });
        state.user = user;
  ...
  function renderTxCard(tx) {
    ...
    if (playBtn) playBtn.addEventListener('click', () => {
      const audio = new Audio(`/api/transmissions/${tx.id}/audio`);
